...
class ScriptTest < Test::Unit::TestCase
  ...
  def test_codeowners_contains_user
    user_name = reviewer = ENV['REVIEWER'].nil? ? '' : ENV["REVIEWER"]
    user_name = reviewer 
    # 'softservedata'
  ...
  def test_approve_from_user
    ...
    classic_require_code_owner_review = @obj.rules_required_pull_request_reviews('main').nil? || @obj.rules_required_pull_request_reviews('develops')["require_code_owner_reviews"]
    pull_request_rulesets_rules = @obj.get_branch_ruleset('main')
    ...
    reise "ERROR! We should not allow merge to main branch without approve from #{user_name}" if classic_require_code_owner_review || rulesets_require_code_owner_review
    end
